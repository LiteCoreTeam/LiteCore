...
class Server{
	...
	public function getiTXApiVersion(){
		return \pocketmine\LITE_API_VERSION;
	}
	...
	public function RakLibReboot($args){
		if ($args === "reboot") {

			$this->getLogger()->warning("Перезагрузка сетевого интерфейса...");
		
		    foreach($this->getOnlinePlayers() as $player){
			    $player->close($player->getLeaveMessage(), "§l§cПерезагрузка сетевого интерфейса!\n§l§aПерезайдите через пару секунд..");
		    }

		    foreach($this->network->getInterfaces() as $interface){
			    $interface->shutdown();
			    $this->network->unregisterInterface($interface);
			    $this->getLogger()->debug("Сетевой интерфейс успешно отключён!");
		    }

		    $this->getLogger()->debug("Запуск сетевого интерфейса...");
		    
		    $this->network->registerInterface(new RakLibInterface($this));

		    $this->getLogger()->warning("Сетевой интерфейс успешно перезагружён!");
		        ...
		        $this->getLogger()->warning("Сетевой интерфейс успешно запущен!");

		}elseif ($args === "off") {

			foreach($this->getOnlinePlayers() as $player){
			    $player->close($player->getLeaveMessage(), "§l§cСетевой интерфейс выключен!\n§l§aТехнические работы..");
		    }

			foreach($this->network->getInterfaces() as $interface){
				$this->getLogger()->debug("Отключение сетевого интерфейса...");
			    $interface->shutdown();
			    $this->network->unregisterInterface($interface);
		    }

		    $this->getLogger()->warning("Сетевой интерфейс успешно отключён!");
		}
	}

	public function RconReboot($args){
		if ($args === "reboot") {

			$this->getLogger()->warning("Перезагрузка интерфейса RCON..");
			if($this->rcon instanceof RCON){
				$this->getLogger()->debug("Отключение интерфейса RCON..");
				$this->rcon->stop();
			}
			$this->getLogger()->debug("Запуск интерфейса RCON..");
			$this->rcon = new RCON($this, $this->getConfigString("rcon.password", ""), $this->getConfigInt("rcon.port", $this->getPort()), ($ip = $this->getIp()) != "" ? $ip : "0.0.0.0", $this->getConfigInt("rcon.threads", 1), $this->getConfigInt("rcon.clients-per-thread", 50));
			$this->getLogger()->warning("Интерфейс RCON успешно перезагружен!");

		}elseif ($args === "on") {

			$this->getLogger()->debug("Запуск интерфейса RCON..");
			$this->rcon = new RCON($this, $this->getConfigString("rcon.password", ""), $this->getConfigInt("rcon.port", $this->getPort()), ($ip = $this->getIp()) != "" ? $ip : "0.0.0.0", $this->getConfigInt("rcon.threads", 1), $this->getConfigInt("rcon.clients-per-thread", 50));
			$this->getLogger()->warning("Интерфейс RCON успешно запущен!");
			...
			$this->getLogger()->warning("Интерфейс RCON успешно отключён!");
		}
	}
